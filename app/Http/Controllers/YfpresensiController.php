...
class YfpresensiController extends Controller
{
    ...
    public function khusus_store(Request $request)
    {
        $lock = Lock::find(1);
        if ($lock->upload) {
            // $this->clear_upload_lock();
        ...
        foreach ($karyawanHadir as $kh) {
            ...
            $no_scan = khusus_noScan($first_in, $first_out, $second_in, $second_out, $overtime_in, $overtime_out);
            $late = khusus_late_check_detail($first_in, $first_out, $second_in, $second_out, $overtime_in, $shift, $tgl, $kh->user_id);
            $dataKaryawan = Karyawan::where('id_karyawan', $user_id)->first();
            // dd($kh->user_id, $late);
            // if ($kh->user_id == 935) dd($kh->user_id, $late);
            if ($dataKaryawan == null) {
                $id_karyawan = 'kosong';
            } else {
                $id_karyawan = $dataKaryawan->id;
            ...
            if ($no_scan != null) $late = null;
            // lanjutkan
            $hasil = khusus_saveDetail($user_id, $first_in, $first_out, $second_in, $second_out, $late, $shift, $tgl, $dataKaryawan->jabatan_id, $no_scan, $dataKaryawan->placement_id, $overtime_in, $overtime_out);
            // dd($hasil['jam_kerja']);
            $total_hari_kerja = 0;
            $total_jam_kerja = 0;
            $total_jam_lembur = 0;
        ...
        Yfpresensi::query()->truncate();
        $this->clear_upload_lock();
        // dd('done khusus', $jumlahKaryawanHadir);
        // return back()->with('info', 'Berhasil Import : ' . $jumlahKaryawanHadir . ' data');
        return redirect('/yfupload')->with('info', 'Berhasil Import : ' . $jumlahKaryawanHadir . ' data');
    }
}
